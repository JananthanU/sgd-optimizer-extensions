...
class MSELossNode(MetaNode):
    ...
    def __init__(self, y_hat: ValueNode, y: ValueNode, out: ValueNode):
        super().__init__()
        y_hat.connect_to(self)   # parents[0] = y_hat
        y.connect_to(self)       # parents[1] = y
        self.connect_to(out)
        self._received_count = 0

    def get_parent_values(self) -> tuple[float, float]:
        return self.parents[0].v, self.parents[1].v   # y_hat, y

    def receive_parent_value(self, v: float): 
        del v
        self._received_count += 1
        if self._received_count == 2: # waiting for both y_hat and y to be received before marking ready
            self.input_ready = True
    ...
    def forward(self):
        if self.input_ready:
            y_hat, y = self.get_parent_values() # get y_hat and y values from parents
            j = 0.5 * (y_hat - y) ** 2 
            for node in self.children: 
                node.receive_parent_value(j) # push the loss value to children
                node.forward() # continue forward pass to children

    def backward(self, grad_z): # self_parents[0] = y_hat, self.parents[1] = y
        y_hat, y = self.get_parent_values() # get y_hat and y values from parents
        self.parents[0].backward(grad_z * (y_hat - y))   # ∂j/∂y_hat 
        self.parents[1].backward(grad_z * -(y_hat - y))  # ∂j/∂y   